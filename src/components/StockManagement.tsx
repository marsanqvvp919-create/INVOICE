...
export default function StockManagement({
  ...
  const [stockSortKey, setStockSortKey] = useState<'nameEn' | 'nameJa' | 'sku' | 'stock'>('nameEn');
  const [stockSortOrder, setStockSortOrder] = useState<'asc' | 'desc'>('asc');

  const handleStockSort = (key: 'nameEn' | 'nameJa' | 'sku' | 'stock') => {
    if (stockSortKey === key) {
  ...
  const filteredProductOverview = [...products]
    .filter(p => {
    ...
    .sort((a, b) => {
      if (stockSortKey === 'stock') {
        const stockA = lots.filter(l => l.productId === a.id).reduce((sum, l) => sum + (l.currentStock || 0), 0);
        const stockB = lots.filter(l => l.productId === b.id).reduce((sum, l) => sum + (l.currentStock || 0), 0);
        return stockSortOrder === 'asc' ? stockA - stockB : stockB - stockA;
      }

      let valA = (a[stockSortKey] || '').toString().toLowerCase();
      let valB = (b[stockSortKey] || '').toString().toLowerCase();
          ...
          <div className="p-4 space-y-4">
            ...
            <div className="flex flex-col sm:flex-row justify-between gap-3 items-stretch sm:items-center">
              ...
              <div className="flex items-center gap-2 shrink-0 self-end sm:self-auto">
                <span className="text-xs font-bold text-slate-500 whitespace-nowrap">ソート:</span>
                <select
                  value={stockSortKey}
                  onChange={(e) => setStockSortKey(e.target.value as any)}
                ...
                >
                  ...
                  <option value="nameJa">製品名 (日本語順)</option>
                  <option value="sku">SKU順</option>
                  <option value="stock">総現在庫数順</option>
                </select>

                <button
                  type="button"
                  onClick={() => setStockSortOrder(prev => prev === 'asc' ? 'desc' : 'asc')}
                  className="bg-slate-50 hover:bg-slate-100 border border-slate-200 text-slate-700 px-2.5 py-1.5 rounded-lg text-xs font-bold flex items-center gap-1 transition-colors cursor-pointer"
                  title={stockSortOrder === 'asc' ? '昇順' : '降順'}
                >
                  {stockSortOrder === 'asc' ? (
                    <>
              ...
              <table className="w-full text-left text-xs border-collapse">
                <thead>
                  <tr className="bg-slate-50 text-slate-500 font-bold border-b border-slate-100 select-none">
                    ...
                    <th 
                      onClick={() => handleStockSort('sku')}
                      className="px-4 py-2.5 w-28 cursor-pointer hover:bg-slate-100 transition-colors"
                    >
                      <div className="flex items-center gap-1">
                        <span>SKU / 製品ID</span>
                        {stockSortKey === 'sku' ? (
                          stockSortOrder === 'asc' ? <ArrowUp className="w-3 h-3 text-blue-600" /> : <ArrowDown className="w-3 h-3 text-blue-600" />
                        ) : (
                    ...
                    <th 
                      onClick={() => handleStockSort('nameJa')}
                    ...
                    <th 
                      onClick={() => handleStockSort('nameEn')}
                      className="px-4 py-2.5 cursor-pointer hover:bg-slate-100 transition-colors"
                    >
                      <div className="flex items-center gap-1">
                        <span>製品名 (英語 / インボイス用)</span>
                        {stockSortKey === 'nameEn' ? (
                          stockSortOrder === 'asc' ? <ArrowUp className="w-3 h-3 text-blue-600" /> : <ArrowDown className="w-3 h-3 text-blue-600" />
                        ) : (
                          <ArrowUpDown className="w-3 h-3 text-slate-300" />
                    ...
                    <th 
                      ...
                      className="px-4 py-2.5 text-right w-36 cursor-pointer hover:bg-slate-100 transition-colors"
                    >
